...
ir       = gamma.*wn.*etam.*fuelheat.*etas; % rxn intensity,btu/ft^2 min
...
r_0      = ir.*xifr./(rhob.*qig.*epsilon);  % default spread rate in ft/min
...
cspeed = speed;
cspdms = max(cspeed,0.);
cros = max(.03333,1.2974 * cspdms.^1.41);       % spread rate, m/s
cros = cros.*ichap;
